...
We use sinon to setup spies for both the Redux Middleware API's `next` function and our own `reportMiddleware`'s `callback` function.  It's worth nothing at `sinon.spy()` returns a normal function which can be used interchangeably.
